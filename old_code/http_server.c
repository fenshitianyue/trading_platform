...
void HandlerFilePath(const char* url_path, char* file_path){
  ...
  sprintf(file_path, "/home/zanda/Desktop/Project/trading_platform/www%s", url_path);
...
int WriteStaticFile(int new_sock, char* file_path){
  ...
  //需要注意的是：这个函数第一个参数必须是一个 socket 文件
...
void HandlerRequest(int64_t new_sock){
  ...
  // d)读取并解析 header 部分（简略考虑，只保留content_length）
